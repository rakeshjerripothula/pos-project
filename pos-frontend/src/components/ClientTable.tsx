      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b-2 border-gray-200">
            ...
            <th className="px-2 py-2.5 text-xs font-semibold text-left text-gray-700">Client Name</th>
            <th className="px-2 py-2.5 text-xs font-semibold text-left text-gray-700">Enabled</th>
            {!isUserOperator && (
        ...
        <tbody>
          {clients.map((c) => (
            <tr key={c.id} className="border-b border-gray-100">
              <td className="px-2 py-2.5 text-sm">{c.id}</td>
              <td className="px-2 py-2.5">
                {editingId === c.id ? (
                  ...
                  <span
                    onClick={() => startEdit(c)}
                    className="inline-block px-2 py-1 text-sm transition-colors rounded cursor-pointer hover:bg-gray-100"
                  >
                    {c.clientName}
                  </span>
                )}
                ...
                <td className="px-2 py-2.5">
                  {editingId === c.id ? (
                    ...
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => startEdit(c)}
                        className="px-2.5 py-1 text-xs text-white transition-colors bg-blue-500 rounded hover:bg-blue-600 cursor-pointer"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => onToggle(c.id, !c.enabled)}
                        className={`px-2.5 py-1 text-xs text-white rounded transition-colors cursor-pointer ${
                          c.enabled 
